...
module Alipay
	...
	def self.wap_trade_auth_url(options, req_options)
	...
	def self.create_req_data(options)
	...
	#产生签名sign
	def self.generate_wap_sign(options)
	...
	def self.get_trade_url(options, token)
	...
	def self.create_req_data_2(token)
